...
class LocationConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        ...
        logger.info(f'LocationConsumer connected: bus_id={self.bus_id}, user={getattr(user, "id", None)}')
        
        # Notify all users that driver is now tracking
    ...
    async def disconnect(self, close_code):
        ...
        logger.info(f'LocationConsumer disconnected: bus_id={self.bus_id}, code={close_code}')
    ...
    async def receive(self, text_data=None, bytes_data=None):
        ...
        if data.get('type') == 'location':
            ...
            if saved:
                logger.info(f'✅ Location saved: bus_id={self.bus_id}, lat={lat}, lng={lng}')
            ...
            try:
                ...
                if route:
                    ...
                    if stops:
                        ...
                        nearest_idx = min(
                            range(len(stops)), 
                            key=lambda i: haversine(lat_f, lng_f, stops[i].latitude, stops[i].longitude)
                        )
                        ...
                        if bus.eta_smoothed_seconds:
                            alpha = 0.3
                            bus.eta_smoothed_seconds = alpha * bus.eta_seconds + (1 - alpha) * bus.eta_smoothed_seconds
                        else:
                            bus.eta_smoothed_seconds = float(bus.eta_seconds)
                        
                        bus.save(update_fields=['nearest_stop_index', 'eta_seconds', 'eta_smoothed_seconds'])
                        logger.info(f'Bus nearest stop: index={nearest_idx}, ETA={bus.eta_seconds}s')
            except Exception as e:
...
class ChatConsumer(AsyncWebsocketConsumer):
            ...
            try:
                ...
                if is_driver:
                    ...
                    if not connected_via_legacy_user_room:
                        ...
                        logger.info('ChatConsumer: user %s connected via legacy room %s', user.id, self.room_name)
            except Exception:
                logger.exception('Failed to add user %s to personal group', getattr(user, 'id', None))

        await self.accept()
        logger.info('ChatConsumer.connect: user_id=%s room_name=%s', getattr(user, 'id', None), self.room_name)
    ...
    async def disconnect(self, close_code):
        ...
        user = self.scope.get('user')
        logger.info('ChatConsumer.disconnect: user_id=%s room_name=%s code=%s', 
                   getattr(user, 'id', None), self.room_name, close_code)
    ...
    async def receive(self, text_data=None, bytes_data=None):
        ...
        logger.info('ChatConsumer.receive: user=%s data=%s', 
                   getattr(self.scope.get('user'), 'id', None), data)
            ...
            try:
                ...
                if not is_sender_driver:
                    ...
                    if bus_obj and bus_obj.driver and bus_obj.driver.user:
                        ...
                        logger.info('ChatConsumer.resolved_recipient_from_bus: sender=%s bus=%s recipient=%s', 
                                  sender.id, bus_id, recipient_id)
                else:
            ...
            try:
                ...
                if is_sender_driver:
                    def _get_last_user_to_driver(sid):
                        last = Message.objects.filter(recipient__id=sid).exclude(
                            sender__id=sid
                    ...
                    if last_user:
                        ...
                        logger.info('ChatConsumer resolved recipient for driver %s -> user %s', 
